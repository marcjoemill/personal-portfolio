...
export default function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    // ── Get the alt text (skill name) from the img ──────────────────────────
    const altText = (el.querySelector('img') as HTMLImageElement)?.alt || '';
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.style.cssText = `
        position:absolute;
        left:${overlayRelRoot.left}px;top:${overlayRelRoot.top}px;
        width:${overlayRelRoot.width}px;height:${overlayRelRoot.height}px;
      ...
      if (originalImg) {
        const imgClone = originalImg.cloneNode() as HTMLImageElement;
      ...
      requestAnimationFrame(() => {
        animatingOverlay.style.left    = `${origRelRoot.left}px`;
        animatingOverlay.style.top     = `${origRelRoot.top}px`;
        animatingOverlay.style.width   = `${origRelRoot.width}px`;
        animatingOverlay.style.height  = `${origRelRoot.height}px`;
